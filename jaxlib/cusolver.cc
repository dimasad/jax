...
std::pair<int, py::bytes> BuildPotrfDescriptor(const py::dtype& dtype,
  ...
  JAX_THROW_IF_ERROR(handle.status());
  int lwork;
...
absl::Status Potrf_(cudaStream_t stream, void** buffers, const char* opaque,
  ...
  JAX_RETURN_IF_ERROR(handle.status());
    ...
    switch (d.type) {
      ...
      case Type::C64: {
        JAX_RETURN_IF_ERROR(JAX_AS_STATUS(cusolverDnCpotrfBatched(
            handle->get(), d.uplo, d.n, static_cast<cuComplex**>(workspace),
            d.n, info, d.batch)));
        break;
      }
      case Type::C128: {
...
absl::Status Gesvd_(cudaStream_t stream, void** buffers, const char* opaque,
  ...
  switch (d.type) {
    case Type::F32: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(JAX_AS_STATUS(cusolverDnSgesvd(
            handle->get(), d.jobu, d.jobvt, d.m, d.n, a, d.m, s, u, d.m, vt,
            d.n, static_cast<float*>(work), d.lwork,
            /*rwork=*/nullptr, info)));
        a += d.m * d.n;
        s += std::min(d.m, d.n);
        u += d.m * d.m;
    ...
    case Type::F64: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(JAX_AS_STATUS(cusolverDnDgesvd(
            handle->get(), d.jobu, d.jobvt, d.m, d.n, a, d.m, s, u, d.m, vt,
            d.n, static_cast<double*>(work), d.lwork,
            /*rwork=*/nullptr, info)));
        a += d.m * d.n;
        s += std::min(d.m, d.n);
        u += d.m * d.m;
    ...
    case Type::C64: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(JAX_AS_STATUS(
            cusolverDnCgesvd(handle->get(), d.jobu, d.jobvt, d.m, d.n, a, d.m,
                             s, u, d.m, vt, d.n, static_cast<cuComplex*>(work),
    ...
    case Type::C128: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(JAX_AS_STATUS(cusolverDnZgesvd(
            handle->get(), d.jobu, d.jobvt, d.m, d.n, a, d.m, s, u, d.m, vt,
            d.n, static_cast<cuDoubleComplex*>(work), d.lwork,
            /*rwork=*/nullptr, info)));
        a += d.m * d.n;
        s += std::min(d.m, d.n);
        u += d.m * d.m;
