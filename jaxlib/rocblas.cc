...
absl::Status Trsm_(hipStream_t stream, void** buffers, const char* opaque,
  ...
  if (buffers[2] != buffers[1]) {
    JAX_RETURN_IF_ERROR(AsStatus(hipMemcpyAsync(buffers[2], buffers[1],
  ...
  if (d.batch == 1) {
    switch (d.type) {
      case Type::F32: {
        ...
        JAX_RETURN_IF_ERROR(AsStatus(rocblas_strsm(handle->get(), d.side, d.uplo, d.trans,
      ...
      case Type::F64: {
        ...
        JAX_RETURN_IF_ERROR(AsStatus(rocblas_dtrsm(handle->get(), d.side, d.uplo, d.trans,
                                         d.diag, d.m, d.n, &alpha,
...
absl::Status Potrf_(hipStream_t stream, void** buffers, const char* opaque,
  ...
  if (buffers[1] != buffers[0]) {
    JAX_RETURN_IF_ERROR(AsStatus(hipMemcpyAsync(buffers[1], buffers[0],
...
absl::Status Orgqr_(hipStream_t stream, void** buffers, const char* opaque,
  ...
  if (buffers[2] != buffers[0]) {
    JAX_RETURN_IF_ERROR(AsStatus(hipMemcpyAsync(buffers[2], buffers[0],
                                SizeOfType(d.type) * d.batch * d.m * d.n,
                                hipMemcpyDeviceToDevice, stream)))
  }
  ...
  if (d.batch == 1) {
    switch (d.type) {
      case Type::F32: {
        ...
        JAX_RETURN_IF_ERROR(AsStatus(rocsolver_sgesvd(handle->get(), d.jobu, d.jobvt, d.m,
      ...
      case Type::F64: {
        ...
        JAX_RETURN_IF_ERROR(AsStatus(rocsolver_dgesvd(handle->get(), d.jobu, d.jobvt, d.m,
                                            d.n, a, lda, s, u, ldu, vt, ldv, e,
                                            rocblas_inplace, info)))
        break;
      }
      case Type::C64: {
        ...
        rocblas_float_complex* vt =
            static_cast<rocblas_float_complex*>(buffers[4]);
        float* e = static_cast<float*>(buffers[6]);
        JAX_RETURN_IF_ERROR(AsStatus(rocsolver_cgesvd(handle->get(), d.jobu, d.jobvt, d.m,
                                            d.n, a, lda, s, u, ldu, vt, ldv, e,
                                            rocblas_inplace, info)))
        break;
      }
      case Type::C128: {
        ...
        rocblas_double_complex* vt =
            static_cast<rocblas_double_complex*>(buffers[4]);
        double* e = static_cast<double*>(buffers[6]);
        JAX_RETURN_IF_ERROR(AsStatus(rocsolver_zgesvd(handle->get(), d.jobu, d.jobvt, d.m,
                                            d.n, a, lda, s, u, ldu, vt, ldv, e,
                                            rocblas_inplace, info)))
        break;
      }
